...
class Graph:
    def __init__(self, filename,weight):
        ...
        if filename is not None:
            ...
            exit()
        if weight == "no" and len(data) > 3:
            warnings.warn("Please make sure 4th column in input is standard deviation generated by BAR ")
        if weight != "no" and len(data) == 3:
            warnings.warn("Only moleculars and pair-wise energy provided. Performing the original cycle closure")
    ...
    def addEdge(self, data,weight):
        ...
        if (weight=="no" and len(data)>3) or weight=="bar":#if bar_std is provided, compare bar_std with single pair std
